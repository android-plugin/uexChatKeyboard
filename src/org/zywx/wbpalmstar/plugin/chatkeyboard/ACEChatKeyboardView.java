...
public class ACEChatKeyboardView extends LinearLayout implements
    ...
    public ACEChatKeyboardView(Context context, JSONObject params,
            EUExChatKeyboard uexBaseObj) {
        super(context);
    ...
    private void initKeyboardParams(JSONObject json) {
        try {
            ...
            if (mTouchDownImg != null) {
                ...
                mTouchDownImgDefaule = getResources().getDrawable(
                        CRes.plugin_chatkeyboard_voice_recording);
            ...
            if (json.has(EChatKeyboardUtils.CHATKEYBOARD_PARAMS_JSON_KEY_DRAGOUTSIDEIMG)) {
                ...
                Bitmap mDragOutsideImgBitmap = mUexBaseObj
                        .getBitmap(dragOutsideImg);
                if (mDragOutsideImgBitmap != null) {
                    mDragOutsideImg = new BitmapDrawable(getResources(),
                            mDragOutsideImgBitmap);
                }
            }
            if (mDragOutsideImg == null) {
                mDragOutsideImgDefaule = getResources().getDrawable(
        ...
        mLayoutTransition.addTransitionListener(new TransitionListener() {
            ...
            public void endTransition(LayoutTransition transition,
                ...
                if (view.getId() == CRes.plugin_chatkeyboard_parent_layout
                    ...
                    goScroll(0);
                    jsonKeyBoardShowCallback(isKeyBoardVisible
                            || mPagerLayout.isShown() ? 1 : 0);
    ...
    public void onDestroy() {
        try {
            ...
            mParentLayout.getViewTreeObserver().removeOnGlobalLayoutListener(
                    this);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private class EmotjiconsPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            View layout = ((Activity) getContext()).getLayoutInflater()
                    .inflate(CRes.plugin_chatkeyboard_emojicons_grid, null);
             ...
             * layout.setPadding(layout.getPaddingLeft(), (int) (keyBoardHeight
             * / 250 * 25), layout.getPaddingRight(),
    ...
    public class EmojiconsGridAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            View layout = ((Activity) getContext()).getLayoutInflater()
                    .inflate(CRes.plugin_chatkeyboard_emojicons_item, null);
    ...
    private class SharesPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            View layout = ((Activity) getContext()).getLayoutInflater()
                    .inflate(CRes.plugin_chatkeyboard_shares_grid, null);
    ...
    public class SharesGridAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView,
                ViewGroup parent) {
            View layout = ((Activity) getContext()).getLayoutInflater()
                    .inflate(CRes.plugin_chatkeyboard_shares_item, null);
    ...
    private void handleRecordWhenDown() {
        ...
        android.view.ViewGroup.LayoutParams viewParams = mRecordTimes
                .getLayoutParams();
        ((MarginLayoutParams) viewParams).setMargins(
                (int) (0.65f * imageWidth), (int) (0.55f * imageHeight), 0, 0);
    ...
    public void onGlobalLayout() {
        ...
        if (heightDifference > 100) {
            ...
            if (mPagerLayout.isShown()) {
                mPagerLayout.setVisibility(View.GONE);
                /*
                 * backScroll(); goScroll(heightDifference);
                 */
            }
    ...
    private void goScroll(int heightDifference) {
        if (!isKeyboardChange) {
            ...
            int keyboardHeight = mPagerLayout.isShown() ? mPagerLayout
                    .getHeight() : 0;
            int inputHeight = isKeyBoardVisible || mPagerLayout.isShown() ? EUExUtil
                    .dipToPixels(50) : 0;
            int bottomPoint = ((View) mUexBaseObj.mBrwView.getParent())
                    .getBottom();
            int bottomMargin = mParentLayout.getHeight() - bottomPoint;
            Log.i(TAG, "bottomMargin : " + bottomMargin + "   " + bottomPoint
                    + "   " + mParentLayout.getHeight());
            if (bottomMargin > inputHeight) {
                inputHeight = 0;
            }
            int screenHeight = mParentLayout.getRootView().getHeight();
            if (mBrwViewHeight > 0
                    || tempHeight > screenHeight - heightDifference) {
                if (bottomMargin + heightDifference + 3 > inputHeight) {//+3是纠正减法导致的计算误差
                    inputHeight = heightDifference;
                } else {
                    inputHeight = heightDifference + inputHeight;
                }
            }
            Log.i(TAG, "Move! height:"
                    + (tempHeight - keyboardHeight - inputHeight)
                    + " tempHeight:" + tempHeight + " ParentkeyboardHeight:"
                    + keyboardHeight + " inputHeight:" + inputHeight);
    ...
    private void changeKeyBoardHeight(int keyBoardHeight) {
        LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) mPagerLayout
                .getLayoutParams();
